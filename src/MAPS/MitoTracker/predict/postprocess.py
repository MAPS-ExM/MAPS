...
def close_background_holes(img, mitos=None):
    ...
    for c_mito_label in unique_mitos[1:]:
        ...
        # TODO: Find and replace background holes smaller than 500px
...
def process_img(img, fragment_threshold=0.25, plot_hist=False):
    ...
    img, mitos = remove_small_parts(
        img, mitos, threshold=25000
    )  # even though we decided 50k in the Meeting (02.02.2023)
    ...
    # TODO:
    # img = close_background_holes(img)

    if plot_hist:
        pass
        # plt.figure()
        # plt.hist(frag_perc, bins=30)
        # plt.savefig(os.path.join(path_target, 'FragHistograms', file[:-4] + '.png'))
        # plt.clf()
...
def process_file(file, path_target, fragment_threshold=0.25, plot_hist=False):
    ...
    print(
        ...
        "Started: ",
        file,
        f"with threshold {fragment_threshold}",
        flush=True,
    )
    ...
    tifffile.imwrite(
        output_file,  # for experimenting with different thresholds: file[:-4] + f'_{fragment_threshold}.tif')
        img,
        imagej=True,
        metadata={"axes": "ZYX"},
        compression="zlib",
